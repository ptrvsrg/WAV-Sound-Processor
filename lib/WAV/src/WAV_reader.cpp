...
void WAVReader::Open(std::string file_path)
{
    ...
    if (file_path_.find(".wav") == std::string::npos)
        throw ExtensionException(file_path_);
...
void WAVReader::SearchChunk(uint32_t chunk_ID)
{
    ...
    while (true)
    {
        fin_.read((char *)&chunk_header,
                  sizeof(chunk_header));
        if (!fin_.good())
            throw ChunkSearchException(file_path_,
                                       chunk_ID);
...
void WAVReader::ReadHeader()
{
    ...
    if (!fin_.good() || RIFF_header.ID_ != RIFF)
        throw RIFFHeaderException(file_path_);
    ...
    if (!fin_.good() || format_type != WAVE)
        throw FormatTypeException(file_path_);
    ...
    if (!fin_.good())
        throw FormatDataException(file_path_);
